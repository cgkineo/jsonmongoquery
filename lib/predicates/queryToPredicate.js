...
export default function queryToPredicate (queryPart, parentPath = '', path = '') {
  if (!isDocument(queryPart)) {
    return isEqualOrIsEmbeddedEqual(queryPart, { isRegExpAsValue: false })
  }
  ...
  const queryOperators = attributes
    ...
    .map(([name, queryPart1]) => {
      ...
      return [name, QUERY_OPERATORS[name](queryPart1, parentPath, name, queryPart)]
    })
    .filter(Boolean)
  ...
  const absolutes = nonOperators
    ...
    .map(([path, queryPart1]) => {
      return [path, isEqualOrIsEmbeddedEqual(queryPart1, { isRegExpAsValue: false })]
    })
  const descendants = nonOperators
    ...
    .map(([path, queryPart1]) => {
      return [path, queryToPredicate(queryPart1, pathsJoin(parentPath, path), path, queryPart)]
    })
  const updateOperators = attributes
    ...
    .map(([name, queryPart1]) => {
      return [name, UPDATE_OPERATORS[name](queryPart1, parentPath, name, queryPart)]
    })
  return function query (value, path, parent) {
    ...
    const haveQueryOperatorsPassed = queryOperators.every(([, predicate]) => {
      ...
      if (!pathEntries?.length) return predicate(undefined, path, parent)
      return pathEntries?.some(item => predicate(item, path, parent))
    })
    ...
    const haveDescendantsPassed = descendants.every(([path, predicate]) => {
      const pathEntries = pathRead(value, path)
      if (!pathEntries?.length) return predicate(undefined, path, value)
      return pathEntries?.some(item => predicate(item, path, value))
    })
