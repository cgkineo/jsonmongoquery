...
export default function queryToPredicate (queryPart, context = new Context({ value: queryPart })) {
  ...
  const descendants = nonOperators
    ...
    .map(([path, queryPart1]) => {
      return [path, queryToPredicate(queryPart1, context.up(path))]
    })
  ...
  return function query (value, path = '', parent, context = new Context({ value, path, parent })) {
    ...
    updateOperators.forEach(([, updater]) => {
      didUpdate = didUpdate || updater(value, path, parent, context)
    })
    return didUpdate
  }
}
